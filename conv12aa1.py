...
Original file is located at
    https://colab.research.google.com/drive/11jpKiHWQ-K8Ahc8O4nwkiMiidqxfA7Iy
"""
...
class Conv3x3:
  ...
  def __init__(self, num_filters, padding=0):
  ...
  def forward(self, input, padding=0):
    ...
    # Get the number of dimensions in the input shape
    num_dims = len(input_shape)

    # If the input has more than two dimensions, raise an error
    if num_dims != 2:
        raise ValueError("Invalid input shape. Expected 2 dimensions, got {} dimensions.".format(num_dims))
    ...
    for f in range(self.num_filters):
        ...
        for i in range(h):
            for j in range(w):
                # Extract the receptive field from the input
                receptive_field = input[max(i - self.padding, 0):i + self.filter_size[0] + self.padding,
                                        max(j - self.padding, 0):j + self.filter_size[1] + self.padding]
  ...
  def get_weights_fp16(self):
    return self.filters.astype(np.float16)
...
class Conv3x3_1_to_n_padding:
  ...
  def __init__(self, output=1, input=1, padding=0):
    num_filters = output
    in_ch = input
        ...
        for f in range(self.num_filters):
            ...
            for i in range(output_shape[0]):
                for j in range(output_shape[1]):
                    ...
                    # Apply the filter to the receptive field
                    output[i, j, f] = np.sum(receptive_field * self.filters[f][:receptive_field.shape[0], :receptive_field.shape[1]])
